...
import 'package:frontend/features/chatbot/domain/entities/chatbot_entity.dart';
...
import '../../../chatbot/presentation/bloc/chatbot_state.dart';
...
class _ChatPageState extends State<ChatPage> {
  ...
  void _startVideoCall() async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to start video call: $e')));
    }
  }
      ...
      child: BlocListener<ChatbotBloc, ChatbotState>(
        listener: (context, state) {
          if (state is ChatbotLoadedState) {
            final latestMessage = state.messages.lastWhere(
                  (msg) => !msg.isUserMessage,
              orElse: () => ChatbotEntity(id: '', text: '', isUserMessage: false, timestamp: DateTime.now()),
            );
            if (latestMessage.text.isNotEmpty) {
              final chatBloc = context.read<ChatBloc>();
              final socketService = context.read<SocketService>();
              final message = MessageEntity(
                id: DateTime.now().millisecondsSinceEpoch.toString(),
                conversationId: widget.conversationId,
                senderId: '684a366542954defe0131ff7',
              ...
              socketService.socket.emit('sendMessage', botMessage);
              chatBloc.add(ReceiveMessagesEvent({
                '_id': message.id,
                'conversationId': message.conversationId,
                'senderId': message.senderId,
                'text': message.text,
                'image': message.image,
                'createdAt': message.createAt.toIso8601String(),
              }));
              _scrollToBottom();
            }
          }
              ...
              children: [
                CircleAvatar(
                  ...
                  child: widget.profilePic == null || widget.profilePic!.isEmpty
                      ? Text(widget.mate[0].toUpperCase(), style: const TextStyle(color: AppColors.white))
                      : null,
                ),
                SizedBox(width: 10),
                Text(widget.mate),
                ...
                builder: (context) => PopupMenuButton<String>(
                  ...
                  onSelected: (value) {
                    if (value == 'delete') {
                      ...
                      context.push("/add-member-page?conversationId=${widget.conversationId}");
                    }
                  },
                    ...
                    const PopupMenuItem<String>(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 8),
                          Text('Delete Conversation', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w500)),
                        ],
                      ),
                    ),
                    if (_isGroup)
                      const PopupMenuItem<String>(
                        ...
                        child: Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Text('Add Member', style: TextStyle(color: Colors.blue, fontWeight: FontWeight.w500)),
                          ],
                        ),
                      ),
                  ],
                  ...
                  builder: (context, state) {
                    if (state is ChatLoadingState) {
                      ...
                      _filteredMessages = _isSearching
                          ? state.messages
                          .where((message) => message.text.toLowerCase().contains(_searchController.text.toLowerCase()))
                          .toList()
                          : state.messages;
                      ...
                      if (_filteredMessages.isEmpty) {
                        return Center(
                          child: Column(
                            ...
                            children: [
                              Icon(_isSearching ? Icons.search_off : Icons.chat_bubble_outline, size: 64, color: Colors.grey[400]),
                              SizedBox(height: 16),
                              Text(
                                _isSearching ? "No messages found" : "No messages yet",
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (isSentMessage) {
                            return _buildSentMessage(context, message.text, message.image, message.id);
                          } else {
  ...
  Widget _buildReceivedMessage(BuildContext context, String mess, List<String>? images) {
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: mess.startsWith('@TLG_ChatBot') ? Colors.yellow[300] : DefaultColors.receiverMessage,
          borderRadius: BorderRadius.circular(15),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (mess.isNotEmpty) Text(mess, style: Theme.of(context).textTheme.bodyMedium),
  ...
  Widget _buildSentMessage(BuildContext context, String mess, List<String>? images, String messageId) {
    return Align(
            ...
            children: [
              GestureDetector(
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    if (_isUploading) CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Colors.grey)),
                  ],
                ),
              ),
              SizedBox(width: 10),
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: "Message (Try @TLG_ChatBot for chatbot)",
